  ...
  static Future<void> captureAndRecognizeText({
    ...
    try {
      ...
      if (mrzLines.length >= 3) {
        ...
        //String gender = genderCode== 'F' ? 'Erkek' : 'Kadın';

        String yearPart= birthDate.substring(0,2);
        String monthPart= birthDate.substring(2,4);
        String day= birthDate.substring(4,6);

        int surnameEndIndex = line3.indexOf('<'); //ilk < işareti görülene kadar git
        String surname = line3.substring(0, surnameEndIndex); //ilk < işaretine kadar olan karakterleri al


        int nameStartIndex = line3.indexOf('<', surnameEndIndex + 1) + 1; //isim ve soy isim arasında iki adet < var ismin başlangıcını bul
        int nameEndIndex = line3.indexOf('<', nameStartIndex);//isim başlangıcınden son < işaretine kadar olan karakterleri al

        if (nameEndIndex == -1) { // isimden sonra3. satırda ikinci < işareti bulunmaz ise isimin bitişi üçüncü satır uzunluğu olur
        ...
        if(year <= currentYear){ //yıl bilgisi şuanki yıldan küçük ise 2000 ve sonraki doğum tarihleri için
          year+=2000;
        }else{ //yıl bilgisi şuanki yıldan büyük ise 1999 ve önceki doğum tarihleri için
          year+=1900;
        }
            ...
            return AlertDialog(
              ...
              content: Column(
                ...
                children: [
                  Text('İsim: $name'),
                  Text('Soysim: $surname'),
                  Text('Kimlik Numarası: $identificationNumber'),
                  Text('Cinsiyet: $gender'),
                  Text("Doğum Tarihi: $day $month $year"),
                  //Text("cinsiyet:$genderCode ")
